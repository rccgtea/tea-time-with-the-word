...
exports.chat = exports.scheduledDailyScripture = exports.getTodaysScripture = void 0;
...
async function callGenAIForChat(prompt) {
    if (!GENAI_API_KEY)
        throw new Error('GenAI API key not configured');
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(GENAI_MODEL)}:generateContent?key=${encodeURIComponent(GENAI_API_KEY)}`;
    const body = {
...
exports.scheduledDailyScripture = functions.pubsub
